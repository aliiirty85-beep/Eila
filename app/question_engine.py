...
class QuestionEngine:
    def __init__(self,ai,screen,context_registry): self.ai=ai; self.screen=screen; self.context_registry=context_registry
    async def from_gaze(self,gaze:dict,deep=False):
        zone=(gaze.get("zone") or "").lower(); active=self.context_registry.current()
        prompt=("یک micro-goal دقیق فارسی بساز. فقط از محتوای قابل مشاهده/داده‌شده استفاده کن. "
                ...
                "JSON: {study,kind,instruction,question,expected,seconds,topic,hook,confidence}")
        content=[{"type":"text","text":prompt+"
Gaze summary: "+json.dumps(gaze,ensure_ascii=False)}]
        if active and active.get("content"): content.append({"type":"text","text":"Active source context:
