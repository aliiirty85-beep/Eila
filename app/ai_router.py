...
class AIRouter:
    ...
    async def _completion_off_loop(self,model,messages,temperature,max_tokens):
        """LiteLLM/provider SDKs may perform blocking work before yielding.
        Keep every provider call off FastAPI's event loop so Core health/UI remain responsive."""
        def call():
            from litellm import completion
            return completion(model=model,messages=messages,temperature=temperature,
                              max_tokens=max_tokens,timeout=45)
        return await asyncio.wait_for(asyncio.to_thread(call),timeout=50)
    ...
    async def ask(self,tier:str,messages:list[dict],temperature=.2,max_tokens=700):
        ...
        for model in self._models(tier):
            ...
            try:
                r=await self._completion_off_loop(model,messages,temperature,max_tokens)
                self._ok(model)
    ...
    def health(self):
        return {"models":{tier:self._models(tier) for tier in ("fast","deep","vision","judge","research")},
                "blocked_until":self.blocked_until,"failures":self.failures}
