...
class SyncManager:
    ...
    def restore_memory_if_empty(self,device_id:str):
        ...
        for key,item in (s.get("spine") or {}).items():
            ...
            remote_rev=int(item.get("revision") or 0)
            if not local or remote_rev>int(local.get("revision") or 0):
                self.spine.set(key,item.get("value"),str(item.get("source_device") or device_id),
                               expected_revision=int(local["revision"]) if local else 0)
